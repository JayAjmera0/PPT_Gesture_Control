...
def is_laser_detected(handPoints):
    """
    Determines if the hand configuration represents a laser mode.
    
    Parameters:
    handPoints (list of tuples): Coordinates of hand landmarks.
    
    Returns:
    bool: True if laser mode is detected, False otherwise.
    """
    # Check if index and middle fingers are up, indicating laser mode
...
while True:
    ...
    if multiLandMarks:
        ...
        if not writing_mode and not laser_mode:
            ...
            if is_laser_detected(handPoints):
                pyautogui.press('esc')
                laser_mode = False
